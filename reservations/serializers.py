...
class ReservationCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = RoomReservation
        fields = ["reservation_id", "start_date", "end_date", "message"]

    def get_reservation_id(self, object):
        return object.id

    def create(self, validated_data):
        message = validated_data.pop("message")
        reservation = super().create(validated_data)
        reservation.messages.create(author=reservation.user, text=message)
        return reservation
...
class ReservationUpdateSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if not (len(scores) is 6):
            raise ValueError(
                "all 6 score should be sepecified in integer", "and no digits!"
            )
        for score in scores:
